// @ts-nocheck
import DipModal from '@decision-agent/components/DipModal';
import { useEffect, useRef, useState } from 'react';
import { getConversationList, markReadConversation } from '@decision-agent/apis/super-assistant';
import { handleConversationGroup } from '@decision-agent/components/DipChat/assistant';
import styles from './index.module.less';
import classNames from 'classnames';
import SearchInput from '@decision-agent/components/SearchInput';
import ScrollBarContainer from '@decision-agent/components/ScrollBarContainer';
import type { ConversationItemType, DipChatItem } from '@decision-agent/components/DipChat/interface';
import { Button, Spin, Splitter } from 'antd';
import { LoadingOutlined } from '@ant-design/icons';
import { useDipChatStore } from '@decision-agent/components/DipChat/store';
import { useDeepCompareMemo } from '@decision-agent/hooks';
import NoSearchResult from '@decision-agent/components/NoSearchResult';
import BubbleList from '../BubbleList';
import GradientContainer from '@decision-agent/components/GradientContainer';
import ClickView from '@decision-agent/assets/icons/clickView.svg?react';
import ConversationItem from './ConversationItem';
import intl from 'react-intl-universal';

const ConversationListModal = ({ onClose, agentAppKey, startNewConversation }: any) => {
  const {
    dipChatStore: { activeConversationKey, chatList, activeChatItemIndex, activeProgressIndex, streamGenerating },
    cancelChat,
    setDipChatStore,
    resetDipChatStore,
  ...
  const cacheSidebarState = useRef({
    activeChatItemIndex,
    activeProgressIndex,
    streamGenerating,
  });
  ...
  const getDetailsData = async () => {
    ...
    if (res) {
      const { chatList, read_message_index, message_index, conversationLoading } = res;
      // console.log('getDetailsData', chatList);
      setDipChatStore({
        chatList,
        activeChatItemIndex: -1,
        activeProgressIndex: -1,
        streamGenerating: conversationLoading,
      });
  ...
  return (
    <DipModal
      ...
      onCancel={() => {
        onClose();
        setDipChatStore({
          chatList: cacheChatList.current,
          activeChatItemIndex: cacheSidebarState.current.activeChatItemIndex,
          activeProgressIndex: cacheSidebarState.current.activeProgressIndex,
          streamGenerating: cacheSidebarState.current.streamGenerating,
        });
      }}
