...
import {
  ...
  getMyTemplateList,
  exportAgent,
} from '@decision-agent/apis/agent-factory';
import type { Agent, Category } from '@decision-agent/apis/agent-factory/type';
import PlusIcon from '@decision-agent/assets/icons/plus.svg?react';
import { useUserAvatars } from '@decision-agent/hooks/useUserAvatars';
import { useMicroWidgetProps } from '@decision-agent/hooks';
import empty from '@decision-agent/assets/images/empty.png';
import ExportIcon from '@decision-agent/assets/icons/export.svg?react';
import ImportIcon from '@decision-agent/assets/icons/import.svg?react';
import { downloadFile, getFilenameFromContentDisposition } from '@decision-agent/utils/file';
import { useSize } from '@decision-agent/hooks';
import { formatTimeSlash } from '@decision-agent/utils/handle-function/FormatTime';
import { PublishSettingsModal, PublishModeEnum } from '@decision-agent/components/AgentPublish';
import LoadFailed from '@decision-agent/components/LoadFailed';
import SpaceAgentAddButton from '@decision-agent/components/SpaceAgentAddButton';
import BaseCard, { computeColumnCount, gap } from '@decision-agent/components/BaseCard';
import SkeletonGrid from '@decision-agent/components/BaseCard/SkeletonGrid';
...
import ResizeObserver from '@decision-agent/components/ResizeObserver';
import type { CarouselRef } from 'antd/es/carousel';
import { useInfiniteScroll } from 'ahooks';
import SearchInput from '@decision-agent/components/SearchInput';
import { getParam, isJSONString } from '@decision-agent/utils/handle-function';
import { nanoid } from 'nanoid';
import { buildDecisionAgentRoutePath } from '@/pages/DecisionAgent/utils';
...
const DecisionAgent = ({
  ...
  useEffect(() => {
    if (modeFromProps === mode) {
      return;
    }

    nextPaginationMarkerStrRef.current = '';
    setMode(modeFromProps);
    setSelectedCategory({ category_id: '', name: '全部' });
    setSearchName('');
  ...
  const navigateToAgentUsage = ({ id, version, status }: { id: string; version: string; status: string }) => {
    ...
    navigate(buildDecisionAgentRoutePath(location.pathname, `/usage?${qs.stringify(params)}`));
  };

  // 跳转到agent的编辑界面
  const navigateToAgentEditConfig = ({ id }: { id: string }) => {
    let url = buildDecisionAgentRoutePath(location.pathname, `/config?agentId=${id}&hidesidebar=true`);
    const filterParams = getFilterParams();
    if (!_.isEmpty(filterParams)) {
  ...
  const handleCreateClick = () => {
    navigate(buildDecisionAgentRoutePath(location.pathname, '/config?hidesidebar=true'));
  };
  ...
  const handleMenuClick = async ({ key }: { key: string }, agent: Agent) => {
    switch (key) {
      ...
      case TemplateActionEnum.CreateAgentFromTemplate: {
        const url = `/config?templateId=${(agent as any).tpl_id}&mode=createAgent&hidesidebar=true`;
        // const filterParams = getFilterParams();
        // if (!_.isEmpty(filterParams)) {
        //   url += `&filterParams=${encodeURIComponent(JSON.stringify(filterParams))}`;
        // }
    ...
    (agent: any) => {
      switch (mode) {
        case ModeEnum.MyTemplate: {
          // 跳转到编辑模板页面
          let url = buildDecisionAgentRoutePath(
            location.pathname,
            `/config?templateId=${agent.id}&mode=editTemplate&hidesidebar=true`
          );
          const filterParams = getFilterParams();
          if (!_.isEmpty(filterParams)) {
            url += `&filterParams=${encodeURIComponent(JSON.stringify(filterParams))}`;
  ...
  useEffect(() => {
    const fetchPerms = async () => {
      let perms: AgentManagementPermType | undefined;
      try {
        // 获取权限
                      ...
                      <div ref={scrollableRef} style={{ width: 'fit-content' }}>
                        ...
                        {safeCategories.map((category, index) => (
                          <Button
                            ...
                            className={styles.categoryTag}
                            style={index === safeCategories.length - 1 ? { marginRight: 0 } : {}}
