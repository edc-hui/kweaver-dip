...
import { AdMonacoEditor } from '@decision-agent/components/Editor/AdMonacoEditor';
...
import { isJSONString } from '@decision-agent/utils/handle-function';
import Markdown from '@decision-agent/components/Markdown';
import { removeInvalidCodeBlocks } from '@decision-agent/components/Markdown/utils';
...
const SqlToolSide = () => {
  ...
  const progressList = Array.isArray(chatItem?.content?.progress) ? chatItem.content.progress : [];
  const activeProgress: DipChatItemContentProgressType = progressList[activeProgressIndex] || {};
  ...
  const renderOutput = () => {
    ...
    if (typeof outputStr === 'string' && outputStr && isJSONString(outputStr)) {
