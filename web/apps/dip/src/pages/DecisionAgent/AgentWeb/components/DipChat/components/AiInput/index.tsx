...
import { Col, type GetRef, Row, Tooltip, type ButtonProps } from 'antd';
import { CloseCircleFilled, LoadingOutlined } from '@ant-design/icons';
import { useLatestState } from '@decision-agent/hooks';
import { FileTypeIcon, getFileExtension } from '@decision-agent/utils/doc';
...
import type { FileItem } from '@decision-agent/components/DipChat/interface';
import type { SuggestionItems } from './interface';

type SuggestionItemList = Exclude<SuggestionItems, (...args: any[]) => any>;
...
const AiInput = forwardRef<AiInputRef, AiInputProps>((props, ref) => {
  ...
  const checkedFileList = tempFileList.filter(item => item.checked);
  const inputDisabled = !value.inputValue && checkedFileList.length === 0;

  const getSuggestionItems = (items: SuggestionItems): SuggestionItemList => {
          ...
          return (
            <Sender
                ...
                if (!_.isEmpty(suggestions) && nextVal) {
                  ...
                  if (targetSuggestion) {
                    currentSuggestion.current = {
                      ...
                      items: getSuggestionItems(targetSuggestion.items),
                    };
                  }
                  const triggerCharIndex = currentSuggestion.current.triggerCharIndex;
                  if (triggerCharIndex !== -1) {
                    ...
                    if (searchText) {
                      const searchItems = currentSuggestion.current.items.filter(item =>
              ...
              onKeyDown={event => {
                ...
                // 输入框没内容但是上传了文件的时候，按 Enter 允许发送
                if (event.key === 'Enter' && !event.shiftKey && !value.inputValue && checkedFileList.length > 0) {
              ...
              autoSize={autoSize}
              footer={(_, info) => {
                const SendButton = info.components.SendButton;
                const LoadingButton = info.components.LoadingButton;
                const handleSendClick: ButtonProps['onClick'] = () => {
                  handleSubmit();
                };
                return (
