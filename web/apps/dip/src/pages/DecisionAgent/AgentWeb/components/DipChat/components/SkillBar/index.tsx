...
const SkillBar = (props: SkillBarProps) => {
  ...
  return (
    ...
    >
      {icon}
      <div className={classNames(styles.content, 'dip-ml-8 dip-flex-align-center')}>
        <div className={classNames(styles.title, 'dip-ellipsis')} title={title}>
          {title}
        </div>
        <span style={{ lineHeight: 1 }} className={classNames(styles.statusIcon, 'dip-ml-8')}>
          {renderStatusIcon()}
        </span>
      </div>
