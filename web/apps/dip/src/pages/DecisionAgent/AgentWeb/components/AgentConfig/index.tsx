...
import { getToolBoxMarketList } from '@decision-agent/apis/agent-operator-integration';
import { getMCPServerTools } from '@decision-agent/apis/agent-operator-integration/mcp';
  ...
  useEffect(() => {
    const fetchPublishPerm = async () => {
      try {
        ...
        const publishAgentPerm = perms?.agent?.publish ?? false;
        const publishTemplatePerm = perms?.agent_tpl?.publish ?? false;
        const seeTrajectoryAnalysis = perms?.agent?.see_trajectory_analysis ?? false;

        setUserPermissions(prevState => ({
          ...prevState,
          hasPublishPermission: isEditTemplate ? publishTemplatePerm : publishAgentPerm,
          hasTraceAnalysisPermission: seeTrajectoryAnalysis,
        }));
      } catch (ex: any) {
        if (ex?.description) {
