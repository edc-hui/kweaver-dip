...
import AdResizeObserver from '@decision-agent/components/AdResizeObserver/AdResizeObserver';

import type { ITableProps } from '../types';
import useATRH from './TableHooks';

import EmptyIcon from '@decision-agent/assets/icons/empty.svg?react';
...
const ITable: React.FC<ITableProps> = props => {
  ...
  const onResize = _.debounce(() => {
    ...
    const tableTitleDom = tableWrapperRef.current?.querySelector('.dip-table-title');
    const tableHeaderDom = tableWrapperRef.current?.querySelector('.dip-table-thead');
    const tablePaginationDom = tableWrapperRef.current?.querySelector('.dip-pagination'); // 52px
    if (domRefDom) {
