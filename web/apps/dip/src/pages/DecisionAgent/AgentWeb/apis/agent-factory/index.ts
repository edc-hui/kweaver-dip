// @ts-nocheck
import { get, post, del, put, cacheableGet } from '@decision-agent/utils/http';
import qs from 'qs';
import {
  AgentConfig,
...
export const getAgentCategoryList = (): Promise<Category[]> => {
  return get(`${agentFactoryBaseUrl}/category`).then((response: Category[] | { entries?: Category[] } | undefined) => {
